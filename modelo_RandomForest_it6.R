...
myFormula <- lesividad ~ tipo_vehiculo + tipo_persona + edad_desde + prec + vvu
...
entreno$prec = factor(entreno$prec)
entreno$vvu = factor(entreno$vvu)
...
test$prec = factor(test$prec)
test$vvu = factor(test$vvu)
...
entreno <-entreno[!is.na(entreno$prec),]
entreno <- entreno[!is.na(entreno$vvu),]
...
test <-test[!is.na(test$prec),]
test <- test[!is.na(test$vvu),]
